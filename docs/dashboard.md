...
It never changes workflow state, submits payments, or records human decisions.
...
approval reasoning, source text, and recorded evidence are available together.
...
Human decision recording and workflow resumption are not implemented. Content
hashing for duplicate detection was discussed and deliberately deferred because
development repeatedly processes the same fixtures. There is no seen-invoice
flag or duplicate-payment prevention in this prototype.
