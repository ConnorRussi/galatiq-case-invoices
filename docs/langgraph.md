...
deep snapshot of the original `IngestionResult`, semantic, reconciliation, and
database results, three stage-specific critic records, per-stage revision
feedback/count, current stage, database path, and final result. The generic
critic value is internal routing state only; the public `ValidationResult`
exposes stage-specific critic records. The validation graph does not mutate the
ingestion invoice. A
critic-confirmed Semantic DENY never reaches Reconciliation. A critic-confirmed
Reconciliation DENY never reaches Database. Technical failures terminate with
`TECHNICAL_FAILURE` and cannot enter approval.
