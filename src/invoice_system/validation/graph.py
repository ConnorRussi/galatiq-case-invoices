"""LangGraph orchestration for semantic and reconciliation validation."""
...
def _finalize(state: ValidationState, *, status: ValidationStatus, reason: str, denied_by: ValidationStage | None) -> dict:
    ...
    reconciliation_result = state.get("reconciliation_result")
    issues = list(reconciliation_result.issues) if reconciliation_result is not None else list(semantic_result.issues)
    return {"final_result": ValidationResult(status=status, reason=reason, denied_by=denied_by, issues=issues, semantic_result=semantic_result, critic_result=critic_result, ingestion=ingestion, semantic_critic_result=state.get("semantic_critic_result"), reconciliation_result=reconciliation_result, reconciliation_critic_result=state.get("reconciliation_critic_result"))}


def finalize_valid_node(state: ValidationState) -> dict:
    result = _finalize(state, status=ValidationStatus.VALID, reason="reconciliation_pass" if state.get("reconciliation_result") is not None else "semantic_pass", denied_by=None)
    logger.info("[finalize] Validation %s", result["final_result"].status.value)
    return result


def finalize_denied_node(state: ValidationState) -> dict:
    stage = ValidationStage.RECONCILIATION if state.get("reconciliation_result") is not None else ValidationStage.SEMANTIC
...
def build_graph(*, include_reconciliation: bool = False):
    ...
    if include_reconciliation:
        ...
        graph.add_conditional_edges("reconciliation_critic", route_after_reconciliation_critic, {"reconciliation": "reconciliation", "finalize_valid": "finalize_valid_for_phase_1", "finalize_denied": "finalize_denied", "finalize_unresolved": "finalize_unresolved"})
    else:
